...
func TestVarchar_Marshal(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		t.Run("fail", func(t *testing.T) {
			...
			res, err := v.Marshal(givenSize)
			assert.NoError(t, err)
			assert.Equal(t, givenSize, len(res))
			assert.Equal(t, []byte(givenData), res[:len(givenData)])                              // Assert the data is the same removing the padding
			assert.Equal(t, make([]byte, givenSize-len([]byte(givenData))), res[len(givenData):]) // Assert the padding is empty bytes
		})
	})
	t.Run("fail", func(t *testing.T) {
		givenData := "hello world"
...
func TestVarchar_Unmarshal(t *testing.T) {
	t.Run("success - no padding", func(t *testing.T) {
		given := []byte("some-text")
		res, err := new(column_types.Varchar).Unmarshal(len(given), given)
		assert.NoError(t, err)
		assert.NotNil(t, res)
	})
	...
	t.Run("fail", func(t *testing.T) {
		...
		res, err := new(column_types.Varchar).Unmarshal(255, given)
		assert.EqualError(t, err, "(varchar[size=255]) payload bytes are not valid UTF-8")
		assert.Nil(t, res)
	})
}
