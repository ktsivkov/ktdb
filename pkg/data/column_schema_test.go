...
func TestColumnSchema_Bytes__and__LoadColumnSchemaFromBytes(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		t.Run("with default", func(t *testing.T) {
			...
			processorMock := &ColumnProcessorMock{}
			processorMock.On("ReflectionType", ColMock{}.Type(0)).Return(reflect.TypeOf(ColMock{}), nil)
			processorMock.On("FromReflectionType", reflect.TypeOf(ColMock{}), 5, []byte{0xFF, 0x00, 0x00, 0x00, 0x00}).Return(ColMock{}, nil)
			restored, err := data.LoadColumnSchemaFromBytes(processorMock, bytes)
			assert.NoError(t, err)
			assert.Equal(t, original, restored)
		})
	...
	t.Run("fail", func(t *testing.T) {
		t.Run("loading type error", func(t *testing.T) {
			original := &data.ColumnSchema{
				...
				Nullable:   false,
				Default:    nil,
				Type:       reflect.TypeOf(InvalidColMock{}),
			}
			bytes, err := original.Bytes()
			assert.NoError(t, err)
			processorMock := &ColumnProcessorMock{}
			processorMock.On("ReflectionType", InvalidColMock{}.Type(0)).Return(nil, errors.New("error"))
			restored, err := data.LoadColumnSchemaFromBytes(processorMock, bytes)
		...
		t.Run("fail upon default unmarshalling", func(t *testing.T) {
			...
			processorMock.On("FromReflectionType", reflect.TypeOf(InvalidColMock{}), 5, []byte{0xFF, 0x00, 0x00, 0x00, 0x00}).Return(nil, errors.New("error"))
			restored, err := data.LoadColumnSchemaFromBytes(processorMock, bytes)
...
func TestColumnSchema_Validate(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		assert.NoError(t, schema.Validate(ColMock{}))
	})
	t.Run("fail", func(t *testing.T) {
		...
		t.Run("not the same type", func(t *testing.T) {
			...
			assert.EqualError(t, schema.Validate(differentColMock{ColMock{}}), "(column=[name=test-col]) given type [name=col_mock, type=data_test.differentColMock] doesn't match required type [name=col_mock, type=data_test.ColMock]")
		})
	})
}
...
func TestColumnSchema_Unmarshal(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		t.Run("null on nullable", func(t *testing.T) {
			...
			processorMock := &ColumnProcessorMock{}
			processorMock.On("FromReflectionType", ColMock{}.Type(0)).Return(reflect.TypeOf(ColMock{}), nil)
	...
	t.Run("fail", func(t *testing.T) {
		...
		t.Run("error on unmarshal", func(t *testing.T) {
			...
			processorMock := &ColumnProcessorMock{}
			processorMock.On("ReflectionType", ColMock{}.Type(0)).Return(reflect.TypeOf(ColMock{}), nil)
			processorMock.On("FromReflectionType", reflect.TypeOf(ColMock{}), 5, []byte{0x00, 0x00, 0x00, 0x00, 0x00}).Return(nil, errors.New("error"))
			res, err := schema.Unmarshal(processorMock, given)
